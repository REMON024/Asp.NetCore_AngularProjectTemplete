...
namespace NybSys.API.Manager
{
    public class SecurityManager : ISecurityManager, IUserManager, IAccessManager
    {
        ...
        public async Task<IActionResult> GetAllAction(ApiCommonMessage message)
        {
            await _auditLogBLL.SaveLog(nameof(GetAllAccessList), "Get All Controller list", Common.Enums.Action.View, message);

            try
            {
                return Build.SuccessMessage(await _accessRightBLL.GetControllers(p => true));
        ...
        public async Task<IActionResult> GetAllController(ApiCommonMessage message)
        {
            await _auditLogBLL.SaveLog(nameof(GetAllAccessList), "Get All Action list", Common.Enums.Action.View, message);

            try
            {
                return Build.SuccessMessage(await _accessRightBLL.GetActionList(p => true));
        ...
        public virtual async Task<bool> VerifyAccessbyRole(string roleName, string controllerName, string actionName)
        {
            try
            {
                return await _accessRightBLL.VerifyAccessControlByRole(roleName, controllerName, actionName);
            }
            catch (Exception ex)
            {
                return false;
